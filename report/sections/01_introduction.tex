...
This report asks three questions across \emph{three} controlled systems of
increasing difficulty, each with an essentially exact reference free energy:
\begin{enumerate}
  ...
  \item \textbf{What are the limitations, failure modes, and future
        directions?} (Q4)
\end{enumerate}
The three systems are (i) a two-dimensional double-well \emph{metastability}
model with a quadrature-exact reference (\cref{sec:case_meta}); (ii) a
two-dimensional \emph{entropic-bottleneck} model with a narrow transverse
channel, for which both the reference free energy \emph{and} the conditional law
$Y\mid X=x$ are analytic (\cref{sec:case_eb}); and (iii) a many-body
\emph{Weeks--Chandler--Andersen (WCA) dimer} in a dense solvent with a
thermodynamic-integration reference (\cref{sec:case_wca}). Together they span a
range of \emph{how sample-starved ABF is}: from a regime where plain ABF is
already a strong baseline to regimes where it is badly starved on the far side of
a barrier.

Our contributions are: (i) a precise statement of the coupled ABF--FR algorithm
in consistent notation, applicable to both two-dimensional and many-body systems
(\cref{sec:abf,sec:fr}); (ii) three matched-seed production studies that quantify
the effect of FR and of its target marginal on each system
(\cref{sec:case_meta,sec:case_eb,sec:case_wca}); and (iii) a cross-case synthesis
(\cref{sec:synthesis}) that ties the three results into a single picture. A
recurring methodological point is the careful interpretation of an \emph{oracle}
target that uses the reference free energy: it is a diagnostic control that bounds
the achievable benefit, never a deployable method. The central finding, developed
in \cref{sec:synthesis}, is that \emph{FR helps ABF in proportion to how starved
ABF is}: a modest convergence accelerator (or even mildly harmful) when ABF
already samples well, but a large and reliable variance-reduction gain when ABF is
starved --- and that in the starved regimes the precise target \emph{shape}
